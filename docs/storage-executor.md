...
> C언어 초심자를 위한 executor, storage, B+ Tree 인덱스 연결 흐름 안내
...
flowchart LR
    subgraph executor["executor.c — SQL 로직"]
        ...
        E2["SELECT 검증<br/>(컬럼 이름·WHERE 조건)"]
    ...
    subgraph index["bptree.c — 메모리 PK 인덱스"]
        B1["id -> CSV row offset"]
        B2["exact lookup"]
        B3["range scan"]
    end
    ...
    executor -->|"storage_print_row_at_offset()"| storage
    executor -->|"storage_print_rows_where_equals()"| storage
    executor -->|"storage_rebuild_pk_index()"| storage
    executor -->|"bptree_insert/search/visit"| index
```

- **`executor.c`**: SQL 문장 구조체를 받아 타입·이름 검증 후 스토리지에 요청
- **`storage.c`**: 파일 경로, 헤더, CSV 행 입출력만 담당
- **`bptree.c`**: PK `id`로 CSV row offset을 찾는 메모리 인덱스 담당
...
| `SelectStatement` | 파서가 만든 SELECT 문장 구조체. `has_where`, `where_column`, `where_operator`, `where_value` 포함 |
| `LiteralValue` | 숫자/문자열 리터럴과 그 위치 |
| `ErrorInfo` | 오류 메시지와 줄·열 위치 |
| `BPlusTree` | PK `id`로 CSV row offset을 찾는 메모리 인덱스 |
...
flowchart TD
    ...
    RWC["resolve_where_column()"]
    IDX["execute_select_with_index()"]
    RNG["execute_select_with_index_range()"]
    SCN["execute_select_with_scan()"]
    FSC["find_schema_column()"]
...
### 5-2. `storage_print_rows()` (WHERE 없는 SELECT)
...
### 5-3. `storage_print_row_at_offset()` (PK exact lookup)

`WHERE id = 값`으로 B+ Tree에서 offset을 찾은 뒤, CSV 파일에서 해당 row만 읽어 출력합니다.

```mermaid
flowchart TD
    SPO["storage_print_row_at_offset()"]
    OPEN["fopen(rb) — 읽기 모드"]
    SEEK["fseek(offset)"]
    LINE["해당 row 한 줄 읽기"]
    PCL["parse_csv_line()"]
...
### 5-4. `storage_print_rows_where_equals()` (선형 탐색)

이름은 `where_equals`지만 현재는 `=`, `>`, `<`, `!=` 비교를 모두 처리합니다.
PK가 아닌 컬럼 조건, 또는 `id != 값`처럼 인덱스 경로가 아닌 조건은 이 함수가 CSV를 처음부터 끝까지 읽습니다.

```mermaid
flowchart TD
    SPW["storage_print_rows_where_equals()"]
    OPEN["fopen(rb)"]
    HEADER["헤더 검증"]
    LOOP["row 반복"]
    MATCH["csv_value_matches_where()"]
    PRINT{"조건 일치?"}
    OUT["선택 컬럼 출력"]
    NEXT["다음 row"]

    SPW --> OPEN --> HEADER --> LOOP
    LOOP --> MATCH --> PRINT
    PRINT -->|"예"| OUT --> NEXT
    PRINT -->|"아니오"| NEXT
    NEXT --> LOOP
...
### 5-5. `storage_rebuild_pk_index()` (CSV에서 인덱스 복구)

인덱스는 메모리에만 있으므로 프로그램을 다시 실행하면 사라집니다.
그래서 테이블을 처음 만질 때 CSV를 읽어 `id -> row offset`을 다시 넣습니다.

```mermaid
flowchart TD
    SRP["storage_rebuild_pk_index()"]
    OPEN["users.csv 열기"]
    HEADER["헤더 검증"]
    LOOP["row 시작 offset 기록"]
    PARSE["id 컬럼 파싱"]
    INSERT["bptree_insert(id, offset)"]
    MAX["max_id 갱신"]

    SRP --> OPEN --> HEADER --> LOOP
    LOOP --> PARSE --> INSERT --> MAX --> LOOP
```
...
sequenceDiagram
    ...
    ST->>ST: write_csv_row()
    ST-->>EI: row offset + 성공/실패
    EI->>BT: bptree_insert(id, offset)

    EP->>ES: SELECT 문장 전달
    ES->>ES: 스키마 로드
    ES->>ES: resolve_selected_columns()
    ES->>ES: resolve_where_column()
    ES->>BT: PK 조건이면 bptree_search/range scan
    BT-->>ES: offset 또는 offset 목록
    ES->>ST: storage_print_rows*()
    ST->>ST: 헤더 검증 또는 offset row 읽기
    ST-->>ES: 성공/실패
```

executor.c는 "무엇을"만 결정하고, storage.c는 "어떻게 파일에"만 집중합니다.
B+ Tree는 여기에 "어느 row로 바로 갈지"를 알려 주는 인덱스로 붙습니다.
그래서 전체 조회는 기존 CSV 흐름을 유지하고, `WHERE id = 값` 같은 PK 조회만 빠른 경로로 갈 수 있습니다.
